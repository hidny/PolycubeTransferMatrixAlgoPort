...
Final number for N = 36: 71242712815411950635 (3 hours?)
	...
	public static HashMap<Long, PartialGen3> prevConfigs;
	public static HashMap<Long, PartialGen3> curConfigs;
	...
	public static BigInteger solve(int numSquares) {
		...
		for(int width=1; width<=maxWidth; width++) {
			...
			prevConfigs = new HashMap<Long, PartialGen3>();
			...
			for(int length=1; length <= numSquares - width + 1 + 5; length++) {
				...
				for(int i=0; i<width; i++) {
					...
					Iterator<Map.Entry<Long, PartialGen3>> iterator = prevConfigs.entrySet().iterator();					
					
				    while (iterator.hasNext()) {
				    	
				    	Map.Entry<Long, PartialGen3> entry = iterator.next();
						...
						for(int fillSquare=0; fillSquare<2; fillSquare++) {
								...
								if(newTop == ABANDONED_ISOLATED_CELL_POSSIBLY_OK) {
									...
									if( looksGoodSoFar ) {
										...
										if(length > width && origUpperBorderTouched && origLowerBorderTouched) {
											BigInteger numSolutions = new BigInteger("" + prevConfigs.get(curSignature).getEnuration(numSquares));
							...
							if(fillSquare == 0) {
								if(curConfigs.containsKey(newSignature)) {
									curConfigs.put(newSignature, PartialGen3.hardCopyMerge(curConfigs.get(newSignature), prevPartialGen));
								} else {
