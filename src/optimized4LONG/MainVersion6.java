...
// long overflow: (expectedAns - ans)/2^(64-1) = (4415695134978868448596-2146745613912593599828)/2^63 = 246. Looks like it overflowed 246 times...
	...
	public static BigInteger solve(int numSquares) {
		...
		for(int width=1; width<=maxWidth; width++) {
			...
			short curPartail[] = PartialGen6Functions.getInitialPartialGen();
						...
						for(int fillSquare=0; fillSquare<2; fillSquare++) {
								...
								if(newTop == ABANDONED_ISOLATED_CELL_POSSIBLY_OK) {
									...
									if( looksGoodSoFar ) {
										...
										if(length > width && origUpperBorderTouched && origLowerBorderTouched) {
											BigInteger numSolutions = PartialGen6Functions.getEnuration(prevConfigs.get(curSignature), numSquares);
							...
							if(fillSquare == 0) {
								if(curConfigs.containsKey(newSignature)) {
									curConfigs.put(newSignature, PartialGen6Functions.hardCopyMerge(curConfigs.get(newSignature), prevPartialGen));
								} else {
									curConfigs.put(newSignature, PartialGen6Functions.hardCopy(prevPartialGen));
								}
							} else {
								...
								if(maxSquaresAllowedForSignature < PartialGen6Functions.getMinSquares(prevPartialGen) + 1) {
									//Nope
								} else {
									...
									if(curConfigs.containsKey(newSignature)) {
										...
										curConfigs.put(newSignature, PartialGen6Functions.hardCopyMerge(
												curConfigs.get(newSignature),
												PartialGen6Functions.hardCopyAdd1SquareThough(prevPartialGen, maxSquaresAllowedForSignature))
											);
										
									} else {
										
										curConfigs.put(newSignature, PartialGen6Functions.hardCopyAdd1SquareThough(
												prevPartialGen,
