...
public class PartialGen6Functions {
	...
	public static short[] hardCopyMerge(short cur1[], short cur2[]) {
		...
		for(int numSquares=minSquares; numSquares<=maxSquares; numSquares++) {
			...
			long sum = 0L;
			if(first != null) {
				sum += CRTLongUtilFunctions.deriveTotalBasedOnModResidues(first);
			}
			if(second != null) {
				sum += CRTLongUtilFunctions.deriveTotalBasedOnModResidues(second);
			}

			if(CRTLongUtilFunctions.shouldAddNewResidueToDataStruct(sum, numResiduesToUse)) {
		...
		for(int numSquares=minSquares; numSquares<=maxSquares; numSquares++) {
			...
			short tmpStoredResidues[] = CRTLongUtilFunctions.convertLongToResidues(sum, numResiduesToUse);
	...
	public static void printEnumerationsForBoundary(short cur[], int numSquares) {
		...
		for(int i=0; i<numSquares + 1; i++) {
			...
			if(i >= minSquares && i <= maxSquares) {
				...
				System.out.print(CRTLongUtilFunctions.deriveTotalBasedOnModResidues(storedResidues) + ", ");
	...
	public static BigInteger getEnuration(short cur[], int numSquares) {
		...
		if(numSquares >= minSquares && numSquares <= maxSquares) {
			...
			return new BigInteger("" + CRTLongUtilFunctions.deriveTotalBasedOnModResidues(storedResidues));
