...
Final number for N = 39: 4415695134978868448596 ( 7.5 hours from N=1)
	...
	public static BigInteger solve(int numSquares) {
		...
		short residuesForTotal[] = new short[PartialGen7Functions.NUM_RESIDUES];
		
		
		for(int width=1; width<=maxWidth; width++) {
			
			short curPartail[] = PartialGen7Functions.getInitialPartialGen();
						...
						for(int fillSquare=0; fillSquare<2; fillSquare++) {
								...
								if(newTop == ABANDONED_ISOLATED_CELL_POSSIBLY_OK) {
									...
									if( looksGoodSoFar ) {
										...
										if(length > width && origUpperBorderTouched && origLowerBorderTouched) {
											...
											short sumToAdd[] = PartialGen7Functions.getStoredResidues(prevConfigs.get(curSignature), numSquares);
											
											if(sumToAdd != null) {
												if(length == width + 1) {
													
													for(int j=0; j<sumToAdd.length; j++) {
														residuesForTotal[j] = (short)((residuesForTotal[j] + sumToAdd[j]) % CRTUtilFunctions2.primes[j]);
													}

												} else {
	
													for(int j=0; j<sumToAdd.length; j++) {
														residuesForTotal[j] = (short)((residuesForTotal[j] + 2 * sumToAdd[j]) % CRTUtilFunctions2.primes[j]);
													}
												}
											}
										}
							...
							if(fillSquare == 0) {
								if(curConfigs.containsKey(newSignature)) {
									curConfigs.put(newSignature, PartialGen7Functions.hardCopyMerge(curConfigs.get(newSignature), prevPartialGen));
								} else {
									curConfigs.put(newSignature, PartialGen7Functions.hardCopy(prevPartialGen));
								}
							} else {
								...
								if(maxSquaresAllowedForSignature < PartialGen7Functions.getMinSquares(prevPartialGen) + 1) {
									//Nope
								} else {
									...
									if(curConfigs.containsKey(newSignature)) {
										...
										curConfigs.put(newSignature, PartialGen7Functions.hardCopyMerge(
												curConfigs.get(newSignature),
												PartialGen7Functions.hardCopyAdd1SquareThough(prevPartialGen, maxSquaresAllowedForSignature))
											);
										
									} else {
										
										curConfigs.put(newSignature, PartialGen7Functions.hardCopyAdd1SquareThough(
												prevPartialGen,
		...
		BigInteger ret = CRTUtilFunctions2.deriveTotalBasedOnModResidues(residuesForTotal);
